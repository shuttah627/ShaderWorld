...
void main()
{
  ...
  out_fragmentColour = vec4 ( texture2D( texture2, vec2(frequency*s,frequency*t)));
  return;

  if( u_resolution.x > 500.0 )
    out_fragmentColour = vec4( 1.0, 0, 0, 1.0);
  else
    out_fragmentColour = vec4( 0, 1.0, 0, 1.0);
  return;

  // Initialise the base material colour
  vec4 MaterialColour = vec4 (0.7, 0.2, 0.1, 1.0);
  vec4 AmbientColour = vec4 (0.6, 0.6, 0.6, 1.0);

  // Create the chess board pattern in the surface.
  if ((abs (frag.WorldSpaceObjectPosition.x) < 4) && (abs (frag.WorldSpaceObjectPosition.z) < 4))
  {
    float factor = mod (1 + floor (frag.WorldSpaceObjectPosition.x) + floor (frag.WorldSpaceObjectPosition.z), 2);
    MaterialColour = vec4 (0.1, 0.1, 0.1, 1.0) + factor * vec4 (0.9, 0.9, 0.9, 0.0);
  }
